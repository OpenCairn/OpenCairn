...
**Start here:** `01 Now/Strategic Overview.md` - rendered dashboard of what's actively in flight (generated by /morning; the `03 Projects/` root docs are the source of truth)
...
`01 Now/Strategic Overview.md` is a **generated dashboard**, not a task SSOT — `/morning` regenerates it from the `03 Projects/` root docs; never hand-edit it and never put `- [ ]` checkboxes in it. The canonical checkbox lives in the relevant Project page, Area file, or Tickler.

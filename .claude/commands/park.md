...
- **Prescribed checks are claims too — Layer 5 must test whether they can fail.** Where the session wrote a verification step for a *later* run (a dated checkpoint, a "confirm X before Y" instruction, an acceptance criterion), name what that step's stated pass condition would show under the negative hypothesis. A pass condition that holds under both hypotheses is a finding, not a pass: it will manufacture confidence on a schedule, at a point where the session that wrote it is long gone and its reasoning unavailable to correct. The ordinary layers do not reach this — they test what the session asserted, and a prescribed check asserts nothing yet.

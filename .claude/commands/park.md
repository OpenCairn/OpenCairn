...
4. **⚠️ QUALITY GATE: Lint, refactor, proofread modified files (inline)**
   ...
   **⛔ Derive the non-vault half mechanically — recall is not sufficient.** The vault half of this list comes from work you did minutes ago and is easy to recall. The non-vault half is not: config and skill edits happen early, tooling writes files as a side effect, and hooks write files you never touched. This clause has existed for a long time and has still been observed to miss four files in one session — so it needs a trigger, not more emphasis. Run both checks and reconcile their output against what you were about to list:
   ...
   #    that no receipt knows about (survey logs, caches). Recurse: skill bundles,
   #    profiles, and script helpers live in subdirectories, so no -maxdepth here.
   #    Window matches Step 10's.
   find "$CONFIG_DIR/commands" "$CONFIG_DIR/scripts" -type f -mmin -240 2>/dev/null
   find "$CONFIG_DIR" -maxdepth 1 -name '*.log' -type f -mmin -240 2>/dev/null

   # 3. Working tree of every git repo the session touched — run once per repo.
   #    A direct edit to a command/template repo (the canonical edit location for
   #    shared commands) writes no receipt and lives outside $CONFIG_DIR, so
   #    neither check above reaches it.
   git -C <repo> status --short
   ```

   The three checks are deliberately redundant: the receipt carries *descriptions* and reaches outside the config dir (the template repo, a `{VAULT}` script copy); the sweep needs no cooperation from the writing tool and so catches hook-written files no receipt can know about; the repo status catches direct edits that bypass both. None alone is sufficient.

   Anything any check returns that is missing from your list is a file you were about to under-report — add it. Anything they return that another session wrote is **not** yours: exclude it per `_shared-rules.md` §20 (the file list is the attribution boundary, not the mtime window) and say so rather than silently absorbing it.
...
12. **Trace reference graph for status changes (enumerate in main session, delegate propagation to sub-agent):**
   ...
   **Write mechanism.** Stale-reference fixes to planning files go through `locked-edit.sh --replace`, not the Edit tool (see `_shared-rules.md` §5 — including its post-write separator check, which applies to every call this step makes).
   ...
   **(d) Spawn the propagation sub-agent** with `subagent_type: "general-purpose"`, foreground. The prompt must be **self-contained** — every value the sub-agent needs must be embedded verbatim. Include:
   ...
   - **Tool guidance:** "Use `rg --type md -i` (ripgrep, respects `.gitignore`, skips `.git/` auto-save). The `-i` is load-bearing: a session-derived identifier is often lowercase while the stale copy capitalises it (or vice-versa), and the case-sensitive default silently drops such hits — the later case-insensitive closure pass then catches what this step should have. Do not use `grep -r` without exclusions — an unfiltered recursive grep crawls `.git/` and takes minutes on long-lived vaults. If `rg` is not installed, the fallback is `grep -rn -i --include='*.md' --exclude-dir=.git --exclude-dir='06 Archive' <vault> | grep -v -e '/\.git/' -e '/06 Archive/'` — the trailing `| grep -v` filter is belt-and-braces against a `grep` implementation that handles the exclusion flags differently; translate any other rg-syntax globs in this prompt the same way."
...
13. **Route ALL open loops to SSOT:**
   ...
   **Write mechanism.** The Pickup Context section was written at Step 8, so this is an edit to an existing section: re-emit the section with the adjacent-item lines added via `update-session-section.sh "<session log>" N "Pickup Context" --replace` — **not** the Edit tool (session-log edits go through the locking script, same rule as Step 13a and `_shared-rules.md` §5). Carry forward the existing `**For next session:**`, `**Continues:**`, and `**Project:**` lines verbatim in the replacement — `--replace` overwrites the whole section, and dropping the `**Project:**` line invalidates what Step 8b verified. Do this **before** Step 13a, and tell Step 13a the lines are there: 13a's own Pickup Context rewrite must preserve them.
...
13a. **Backfill "Files Updated" in session log:**
   ...
   - **Also reconcile the session log against inline closures.** The `Next Steps / Open Loops` and `Pickup Context` sections were written at Step 8, before Steps 11-13 ran — same staleness root cause as the Files Updated backfill above. If any Next Steps item written at Step 8 is now *done* (e.g. a loop you closed inline at Step 13 rather than routing to a future date), rewrite `### Next Steps / Open Loops` via `update-session-section.sh … N "Next Steps / Open Loops" --replace` with the completed item omitted, and update any `**For next session:**` line in Pickup Context that referenced it. **Preserve any adjacent-open-item lines Step 13's sweep added to Pickup Context** — re-read the section before rewriting it; those lines are the sweep's only durable output, and a `--replace` composed from the Step 8 draft silently drops them. (Distinct from Step 4's VERIFY pass, which only fires when re-parking an already-parked session and runs before routing.) **Why this exists:** caught by audit 2026-06-02 — a memory-update loop listed in Next Steps was closed inline at Step 13, leaving the session log claiming the same task both done (Files Updated) and to-do (Next Steps).

13b. **Bump WIP "Last updated" timestamp:**
   - **Unconditional bump.** Replace the existing `Last updated: ...` line at the top of `01 Now/Works in Progress.md` with the current timestamp (e.g. `Last updated: YYYY-MM-DD HH:MM TZ`) **via `locked-edit.sh --replace`** (per `_shared-rules.md` §5), NOT the Edit tool. Run `date +"%Y-%m-%d %H:%M %Z"` to obtain the value — do NOT reuse Step 1's park-start time or estimate elapsed time (the auto-date reflex writes a fabricated timestamp; caught 2026-06-12). **Two calls, ordered** — the `date` result must appear in a *prior* tool result before you compose the heredoc (`_shared-rules.md` §19, clock values). No "did we modify planning files?" check — by definition something was captured; the imperceptible cost of an over-bumped timestamp on a pure no-write session is preferable to the prediction-failure mode the old conditional produced.
...
14. **Delegate /audit to a fresh sub-agent:**
   ...
   **(b) Spawn the sub-agent** with the Agent tool, `subagent_type: "general-purpose"`. Foreground (not background) — Steps 15-17 need its result.
   ...
   - **⛔ Pre-state authority** — where a finding would turn on what a file looked like *before* this session, the authoritative pre-state is the main session's own Read, not the repository's history. When a status, tick, count, or "was something dropped" claim is in scope, embed the relevant pre-edit content in the brief, and instruct: "Do NOT reconstruct pre-state from the vault's auto-save git. Commit boundaries there are arbitrary and unrelated to session boundaries, so a state that existed only between two commits appears in neither — an item the user changed shortly before this session can read as unchanged in the last commit and be gone from the next. Any git-derived 'silently dropped' or data-loss finding must be verified against the specific commit's diff content and timing, and against the pre-state embedded above, before you remediate it." Without this clause an auditor can produce a confident, fully-remediated finding about work that was never lost — and the remediation itself then corrupts an accurate record. Sibling of `_shared-rules.md` §20: that governs *attribution* from auto-save git (who did what), this governs *pre-state* (what it was before).
   ...
   - **Report format expected back:** "Per-layer findings (or nil-case statements that explicitly name what was checked — generic affirmations like 'approach is sound' are not acceptable); list of remediation edits with file paths + summary of change; bytes-read-per-hub for read-coverage verification; **the evidence attestation per `_shared-rules.md` §23 — paste §23's attestation table into the sub-agent's brief verbatim, since it cannot read this directory**; final clean-pass confirmation OR explicit 'could not clean — N issues remain because X' if iteration stalled."
   - **Enforce the attestation on receipt, per §23.** A report with no attestation is brief-echo — re-despatch rather than accept it. Claims attested but not spot-checked stay `[unverified]` until you check them; this is the same discipline as the remediation sanity-check below, applied to findings rather than edits.
...
16. **Export session transcript:**
   - Export today's verbatim session transcripts to the vault. **This step runs last** so the exported transcript captures the full park including the audit step (Step 14) and any remediation it produced.
     ...
     ```
   - **Use `--all-projects` (cwd-independent).** The transcript file is date-canonical (`YYYY-MM-DD.md`), one per day. A single-project export (the old `cd <launch dir>` + cwd-keyed default) only ever *offers* the launch project's sessions, so on a multi-project day the same-day sessions from *other* project directories never reach the file — it gets provenance-hashed as if final while incomplete. `--all-projects` sweeps every project directory and merges the day, so the export is both complete and cwd-independent — no `cd` needed. (Do NOT use `--fallback-any-project`.)
   - **`--days 7`, not `--days 1` (boundary-day completeness).** The cutoff is a **rolling window measured back from now**, not a calendar-day boundary, and the script dates each transcript file by JSONL **mtime**. Under `--days 1`, a session from *yesterday* that ran more than 24h ago falls outside the window while the rest of its day stays in — so that day is regenerated from only part of its sessions. A 7-day window keeps every session of every touched day inside the cutoff. Matches `/morning` 2a.h + `/weekly-hygiene`; re-exporting 7 days costs <1s.

...
Run `"$VAULT_PATH/.claude/scripts/resolve-vault.sh"`, then `"$VAULT_PATH/.claude/scripts/check-archive-layout.sh" --enforce "$VAULT_PATH"`; abort on either error (usual causes: `VAULT_PATH` unset, or a pending archive migration). Read `_shared-rules.md` from this skill's own commands directory and apply it throughout. `{VAULT}` below = the resolved vault path. Set `PARK_ARTIFACT="{VAULT}/.claude/scripts/park-artifact.py"` and `PARK_ARTIFACT_STATE="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.session-state/.park-artifacts"`; abort if the helper is absent.
...
Run this gate only when the complete argument string is exactly `--quick`, after inventory and before any full reads, persistence, routing, or agent despatch. Supply every repository touched during the session to `park-files.sh`.
...
If any condition fails, print `↪ --quick escalated to full park: [reasons]` and continue at Step 2(b), without rerunning setup or inventory. Otherwise follow the quick success path immediately; do not enter the remaining full-path steps.
...
2. Immediately rerun the same ledger, `park-files.sh`, and local-mutation census. This second inventory—not `park-verify.sh`—detects late files. Permit only the exact assigned session-log mutation. Any new or ambiguously attributed non-bookkeeping delta prints an escalation reason and retains the assigned N: never rerun Step 3; apply Step 2(b)'s proportional classification and inspection to every late created or updated artefact; record late deleted paths from ledger, pre-state, or tool evidence without reading them; replace every affected `Summary`, `Key Insights / Decisions`, `Next Steps / Open Loops`, `Files Created`, `Files Updated`, `Files Deleted`, and `Pickup Context` section via `update-session-section.sh --replace`; complete Step 2(b–e); then resume the full path at Step 4.
3. With the second inventory still nil, re-derive `SESSION_LOG` per Step 0, then run `"{VAULT}/.claude/scripts/park-verify.sh" "{VAULT}" "$SESSION_LOG" N` with no `--ident` or `--touched` arguments. It does not discover omitted files. Repair and rerun only failures confined to the newly assigned session block. A failure involving another file, a pre-existing entry, or any correction outside that block escalates before remediation: retain N, convert every affected sparse section through the same `update-session-section.sh --replace` contract in item 2, and resume the full path at Step 4. Triage any unexpected REVIEW rather than treating it as file discovery.
4. Run the Step 10 skill monitor against the quick execution, then perform Step 11 transcript export. These outputs are Park-owned bookkeeping and do not invalidate eligibility. Skip project-doc mutation, reference-graph propagation and Step 8 collection, open-loop routing, semantic file review/classification, and the independent audit because their inputs were checked nil.
...
- **Local reference artefact** — a manual, textbook, paper, dataset or other source copied/downloaded but not authored this session: hash and identify it, then inspect only passages the session used or claims it supports. Never read the whole source merely because it was touched. Prepare its immutable receipt with `python3 "$PARK_ARTIFACT" --source "<file>" --original "<file>" --state-dir "$PARK_ARTIFACT_STATE"`; text extraction is an index, not evidence every page was reviewed. `text_status: empty` or `unsupported` proves no text coverage, so use targeted rendering/OCR only if a session claim depends on it.
- **Mechanical-only** — exact locator/token substitutions: inspect the exact replacement and changed span, not the whole file.

Across the applicable semantic coverage, fix broken syntax/links/paths, stale interim state, redundancy, typos and spelling per the user's locale, draft-era filename residue after a terminal status change (rename via link-healing move, not raw `mv`), and hook collateral on verbatim external text (§14 — repair via shell, never Edit/Write). If a locale-lint script exists (`.claude/scripts/locale-lint.sh`), run it once over the ledger's Write/Edit rows only — never the mtime backstop's output — and apply only context-valid candidates, always skipping URLs, quoted spans and capitalised runs. Record candidate, applied and skipped counts, including zero. A durable new doc must be linked from a durable parent. For an override, enumerate every imperative in the interval and mark it suspended or retained. Don't auto-revert changes you didn't make — surface them.
...
(e) **Pre-log propagation pass:** the session log is not yet written, so stale living content gets fixed here, in editing mode, rather than recalled later in record-keeping mode. Build the initial Step 6 enumeration now: every `old → new` pair plus, for each named-entity claim, the entity's canonical identifiers. Search each over the live vault in Step 6's search shape (one `rg -F --type md -i` call per identifier, `06 Archive/` excluded), reconcile every stale living hit per §12 (planning/hub writes via `locked-edit.sh`), and report `Pre-log propagation: I identifiers, H hits, U updated`. For a substantive reframe (scope, timeline, audience, constraint, underlying premise), also list the affected project's sibling artefacts (files in its folder, files its hub links, live planning lines that reference it) and read each one's framing for a premise the reframe invalidated; report `Sibling framing: S reviewed, U updated`. Carry this enumeration to Step 4b unchanged and extend it there; Step 6 stays as the backstop, not the primary catch. **New-entity check** (same pass, the complementary failure class: duplication rather than staleness): list every proper noun the session introduced as a new vault concept (person, project, organisation, product, venue, event, place) and run one `rg -F --type md -i` call per name across the whole vault, `06 Archive/` included, since a prior mention under another framing is exactly what this catches. Name each existing home found and merge or cross-link rather than leaving a parallel entry. Report `New entities: E searched` with the hit count per name, zeros included.

Output: `✓ Quality check: N files checked, no issues`, `🔧 Quality check: fixed M issues — [file: fix]`, or — when (a)'s checked inventory is empty — `✓ Quality check: no attributed files (writes deferred to Steps 3–7)`. The empty case is a result, not a skip: planning and research sessions can legitimately make their first writes during Park, and the later writes still pass their step contracts, Step 8 verification, and Step 9 audit.
...
**Do this here — after Step 4, before Step 5 — not at Step 6.** Actually check every Step 6 identifier class against the session's work and tool/world-state effects, extending the Step 2(e) enumeration with everything Steps 3–4 changed (including moves). Display and retain the exact enumeration and category-check results. Checked nonempty → despatch the Step 6 agent now, `run_in_background: true`. Checked-empty → record `Step 4b: checked empty; no propagation agent despatched` and continue without a seat. Unknown/incomplete attribution or any unchecked category is not nil: resolve it before taking either branch; if it cannot be resolved, report the blocker, not a completed Park. File count and a quiet-looking session are never evidence of nil. Step 6 defines the agent's work and prompt; Step 8 rechecks even when no seat was created.
...
**Finalise Step 6 first.** Recompute and display the complete enumeration after all intervening work, including Steps 5–7, using Step 4b's category checks and unknown/incomplete rule. If no seat exists and it is checked-empty, record `✓ Reference graph: No identifier values changed — checked final enumeration; no propagation agent despatched`; neither spawn nor wait, and continue to backfill and verification. If no seat exists and identifiers now exist, despatch the Step 6 background agent with the complete final enumeration and collect it before backfill.

**Only if a propagation seat was actually despatched, collect it as follows; otherwise skip this paragraph.** Its edits belong in the backfill, and park-verify's `--touched` list is incomplete without them. If its report hasn't arrived, wait for the completion notification; do not proceed on the assumption it changed nothing. The notification is the *only* signal it has finished — never substitute a proxy that cannot tell "running" from "finished" (a transcript's file size, an elapsed-time guess, a scratch file appearing). Improvising one has already reported completion mid-run. For an initial seat, compare the final enumeration with the exact despatched one and run and collect a delta propagation pass for added or changed identifiers before backfill. Keep the initial report even when the enumeration is unchanged; no final nil claim may erase work already despatched.

When a seat reported, inspect its returned propagation report for unresolved or gated consequences before backfill. Route each through Step 7. If that routing adds or changes an identifier, run and collect a delta propagation pass for it before continuing; a report that surfaces a new open loop invalidates any earlier "none" result. Apply the same consequence/delta check to each delta report; further identifier changes from propagation or later Park edits require rechecking Step 8 before completion.
...
- **Artefact coverage:** for each bounded semantic hub, provide the full file. For each large semantic or local reference artefact, provide its immutable `park-artifact.py` receipt plus only the exact pages, sections, excerpts or rendered regions used by the session. The reviewer verifies the source hash, any review-copy hash and those targets; it must not full-read a textbook/manual merely because the file appears in the Files list. An extraction covering the full source is an index, not a whole-source read attestation.
- **Protocol:** read `audit.md` Phase 2 (Layers 1–5 — Layer 0 is deliberately out of scope: this delegated bookkeeping audit's frame is fixed by this brief, and there is no requester to reframe to) and Phase 4 step 1 (remediation + deletion discipline), don't recall them. Layer 3 must run the world-state check both ways — work completed but still framed as pending, and work merely scheduled but framed as discharged (check entailed-consequence clauses) — reading bounded semantic hubs in full and using the declared targeted coverage for large/reference artefacts; token lists are illustrative, meaning is the test, and an absent framing is not a finding.

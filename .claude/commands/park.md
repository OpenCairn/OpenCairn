...
14. **Delegate /audit to a fresh sub-agent:**
   ...
   **(b) Spawn the sub-agent** with the Agent tool, `subagent_type: "general-purpose"`. Foreground (not background) — Steps 15-17 need its result.
   ...
   - **Protocol to follow:** "Read `audit.md` Phase 2 (Layers 1-5) first (in `~/.claude/commands/` or `{VAULT}/.claude/commands/`). Do not recall layers from memory."
...
16. **Export session transcript:**
   - Export today's verbatim session transcripts to the vault. **This step runs last** so the exported transcript captures the full park including the audit step (Step 14) and any remediation it produced.
     ```bash
     cd "<session launch directory>" || exit 1   # script keys session discovery on cwd
     python3 "{VAULT}/.claude/scripts/export-session-transcripts.py" "{VAULT}" --days 1
     ```
   - **The `cd` prefix is load-bearing.** The script keys session-dir discovery on its cwd, and the Bash tool's cwd persists across calls — a mid-session `cd` (e.g. into a repo) would make the export find the wrong project. Substitute the session's launch directory — the working directory stated in your environment context, which is static and does not track mid-session `cd`s. Do NOT use `pwd` (it returns the drifted directory, silently reproducing the bug). The script now **fails closed**: if no project dir matches the cwd it errors out (printing the expected `~/.claude/projects/<encoded>` path) instead of exporting an arbitrary project. It also prints `Session directory: <path>` first — confirm that line names the expected project before trusting the export. (`--fallback-any-project` re-enables the old any-project behaviour; do not use it here.)

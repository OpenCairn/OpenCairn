...
12. **Trace reference graph for status changes (enumerate in main session, delegate propagation to sub-agent):**
   ...
   **(d) Spawn the propagation sub-agent** with `subagent_type: "general-purpose"`, foreground. The prompt must be **self-contained** — every value the sub-agent needs must be embedded verbatim. Include:
   ...
   - **Already-updated docs are NOT complete.** "For any identifier flagged in the enumeration as 'already updated in doc X' (a fix the session made inline), do NOT treat doc X as done — re-grep doc X for OTHER instances of the same stale claim. A single in-session edit fixes one occurrence; the same fact is often asserted in several places within one document (summary callout + detail table + rationale prose). Include every session-edited doc in the per-identifier grep pass, not just the unedited rest of the vault."

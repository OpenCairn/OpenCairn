...
(e) **Pre-log propagation pass:** the session log is not yet written, so stale living content gets fixed here, in editing mode, rather than recalled later in record-keeping mode. Build the initial Step 6 enumeration now: every `old → new` pair plus, for each named-entity claim, the entity's canonical identifiers. Search each over the live vault in Step 6's search shape (one `rg -F --type md -i` call per identifier, `06 Archive/` excluded), reconcile every stale living hit per §12 (planning/hub writes via `locked-edit.sh`), and report `Pre-log propagation: I identifiers, H hits, U updated`. For a substantive reframe (scope, timeline, audience, constraint, underlying premise), also list the affected project's sibling artefacts (files in its folder, files its hub links, live planning lines that reference it) and read each one's framing for a premise the reframe invalidated; report `Sibling framing: S reviewed, U updated`. Carry this enumeration to Step 4b unchanged and extend it there; Step 6 stays as the backstop, not the primary catch.

...
10. **Session Transcript Export**
   ...
   Backstop for `/park` step 11 and `/goodnight` step 16, which export daily. This catches any days missed (skipped park/goodnight, crashed session, etc.). Claude Code auto-deletes JSONL session files after `cleanupPeriodDays` (30 by default) — this ensures nothing slips through.
...
14. **Supply-chain config tripwire** (AI-assistant hook integrity)
   ...
   for f in ~/.claude/settings.json ~/.claude/settings.local.json ~/.cursor/*.json ~/.gemini/settings.json; do
     [ -e "$f" ] && rg -l -i 'SessionStart|preinstall|postinstall|binding\.gyp' "$f" 2>/dev/null
   done

...
- **Scrape ladder:** `mcp__firecrawl__firecrawl_scrape` with `waitFor: 5000` if firecrawl MCP is available → else `WebFetch` → else the **`_shared-rules.md` §15 extractor** (needs only `curl`/`pandoc`/`python3`+`bs4`+`lxml`; extracts the body to a file, then Read it into context — also the right rung when the page is too long for a scrape tool) → if all rungs are unavailable or fail, fall to 1b captions (or Tier 2/3); don't stall, note the gap in the final report. This degradation is expected for users without the firecrawl MCP.

...
4. **Run the WhisperX block as a tracked background task, JSON → durable file.** Use `/transcribe` Phase 2's exact Python block with the five variables set (`INPUT_FILE_PATH="$TMP/podcast_digest_$SAFE_ID.<ext>"`, `DIARIZE`, `NUM_SPEAKERS`, `MIN_SPEAKERS=None`, `MAX_SPEAKERS=None`), but **redirect stdout to a durable path and run it via the Bash tool's background mode** (never a shell `&` — a disowned process's exit code is unrecoverable in later tool calls, and the background task reports its own completion):
   ...
   PYEOF
   ```
   On CPU with ~1hr+ audio this is slow — run it as a background Bash task (no timeout). On completion, check the exit code the background task reports; on failure read `$ERR` for diagnostics; on success Read `$JSON_OUT`.

...
**If cloud is chosen — hand off to `/transcribecloud`** (the single runtime delegation this skill allows; re-inlining its large, stateful RunPod pod-provisioning is impractical, so unlike the local core there's no benefit to owning it).
...
- **Use a dedicated output dir** so the result file is unambiguous (`--output "$TMP"` alone is not — `/transcribecloud` names outputs after the source, and `$TMP` may hold prior runs): `CLOUD_OUT="$TMP/podcast_digest_${SAFE_ID}_cloud"`. Invoke with `--output "$CLOUD_OUT"` and `--no-published` (Tier 1 already exhausted the published-transcript search; without the flag its Phase 1.5 re-runs that search and can derail into its own transcript-save flow), plus `--diarize` / `--speakers N` per the Step-3 diarisation *intent* (interview → diarise) — but **only if the Step-3 cloud-branch token check (`[ -s ~/.cache/huggingface/token ]`) said `ok`**: the pod does *not* manage its own token — `/transcribecloud` Phase 3 step 2 transfers the **local** `~/.cache/huggingface/token` and stalls without one, so on `no_token` omit `--diarize`. Pass `--language <code>` if a non-English language is known, or `--language auto` when the language is unknown (its default is `en`). Let its upfront cost/GPU confirmation run — that gate is appropriate for a paid job — but **decline its optional tails** (Phase-8 speaker-rename prompt, synthesis/index offers): the digest does its own attribution and synthesis.
...
```bash
TMP="${TMPDIR:-/tmp}"   # TMPDIR is often unset on Linux; /tmp is the reliable fallback (/transcribe itself hardcodes /tmp)
SAFE_ID="$(printf '%s' "<PRIMARY_URL>" | tr '[:upper:]' '[:lower:]' | sed -E 's#^https?://##; s#[^a-z0-9]+#-#g; s#(^-|-$)##g' | cut -c1-30)-$(printf '%s' "<PRIMARY_URL>" | cksum | cut -d' ' -f1)"
```
This is `_shared-rules-content.md` §15's slug recipe — use it verbatim (same URL string, no trailing-slash or case variation) so the audio file, JSON file, cloud dir, read-back, and cleanup all resolve the same paths.

**Steps:**
1. **Prereq gate (before downloading anything) — local branch only.** On the cloud branch the transcription runs on the pod, so this gate does not apply: skip it and use `/transcribecloud`'s own prerequisites instead (only a download tool is needed locally, for the non-YouTube case). For a local run, verify the transcription toolchain: `~/venvs/whisperx/bin/python3 -c "import whisperx" && command -v ffmpeg && command -v ffprobe`; plus `command -v yt-dlp` (YouTube) and `command -v curl` or `wget` (remote audio). If any is missing, **stop loudly** with the specific install hint — but first offer the **Tier-1 options still untried** (this is not a loop back to a failed tier): the user can always paste a published transcript, and YouTube auto-captions need only `yt-dlp` if Tier 1 was skipped (e.g. for guaranteed speaker labels) rather than exhausted. Never download audio you can't process.
...
3. **Decide diarisation BEFORE running** (this is where delegation would have aborted). Default to diarisation for interviews. Probe the HF token yourself — **local branch:** `~/venvs/whisperx/bin/python3 -c "from huggingface_hub import HfApi; HfApi().whoami()" 2>/dev/null && echo ok || echo no_token`; **cloud branch:** don't route the probe through the local venv (the cloud path may run on a box with no WhisperX install) — check the file `/transcribecloud` actually transfers: `[ -s ~/.cache/huggingface/token ] && echo ok || echo no_token`. If `ok` → `DIARIZE=True`, `NUM_SPEAKERS=2` (or `1` for a known monologue). If `no_token` → `DIARIZE=False`, proceed **non-diarised** (do not abort; the transcript will have no speaker labels — see Phase 2 for best-effort role attribution) — unless the user *explicitly required* speaker labels (the Tier-1-skip case): then don't silently under-deliver — surface the missing token and let them choose between cloud diarisation and a non-diarised run.
4. **Run the WhisperX block as a tracked background task.** Use `/transcribe` Phase 2's exact Python block with the five variables set (`INPUT_FILE_PATH="$TMP/podcast_digest_$SAFE_ID.<ext>"`, `DIARIZE`, `NUM_SPEAKERS`, `MIN_SPEAKERS=None`, `MAX_SPEAKERS=None`), run **via the Bash tool's background mode** (never a shell `&` — a disowned process's exit code is unrecoverable in later tool calls, and the background task reports its own completion). **The block writes the segments JSON to a file of its own choosing and prints only that path** — do not redirect stdout expecting JSON:
   ```bash
   ERR="$TMP/podcast_digest_$SAFE_ID.err"
   ~/venvs/whisperx/bin/python3 2>"$ERR" <<'PYEOF'
   # … /transcribe Phase 2 Python block, writing the JSON to its own path and ending in print(out_path) …
   PYEOF
   ```
   With `INPUT_FILE_PATH` as above, that derived path is `/tmp/transcribe_segments_podcast_digest_$SAFE_ID.json` (the block hardcodes `/tmp`, not `$TMP`) — outside the `$TMP/podcast_digest_$SAFE_ID.*` sweep, so Step 6 removes it explicitly. On CPU with ~1hr+ audio this is slow — run it as a background Bash task (no timeout). On completion, check the exit code the background task reports; on failure read `$ERR` for diagnostics; on success Read the JSON at the printed path.
5. **Format in-context** using `/transcribe` Phase 3 **steps 1–3 only** (parse → format with timestamps (`/transcribe`'s duration-dependent `[MM:SS]`/`[H:MM:SS]`) + any speaker labels → cleanup pass). **Do NOT run step 4 (display), or steps 5–8 (rename/save/discuss prompts).** The digest needs the transcript in context — not dumped to the user, not saved to disk.
6. **Cleanup:** once synthesis is complete remove `$TMP/podcast_digest_$SAFE_ID.*` **and** the segments JSON the Step-4 block wrote outside that glob (`rm -f /tmp/transcribe_segments_podcast_digest_$SAFE_ID.json`) — and, if the cloud path ran, `rm -rf "$CLOUD_OUT"` too.
...
- **Scrape ladder (`_shared-rules-content.md` §26 order):** the **§15 extractor** first (needs only `curl`/`pandoc`/`python3`+`bs4`+`lxml`; extracts the body to a file, then Read it into context — also the right rung when the page is too long for a scrape tool) → else a configured fetch MCP (e.g. `mcp__firecrawl__firecrawl_scrape` with `waitFor: 5000`; a credits/quota error counts as unavailable — move on, don't retry) → else `WebFetch` → if all rungs are unavailable or fail, fall to 1b captions (or Tier 2/3); don't stall, note the gap in the final report. This degradation is expected for installs without any fetch MCP.
...
1. **Scrape the pca.st episode page** (`_shared-rules-content.md` §26 ladder: static extract → fetch MCP → `WebFetch`) for an `og:audio` / enclosure URL. This also harvests episode metadata.

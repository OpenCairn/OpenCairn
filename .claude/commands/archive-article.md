...
Match is **exact, not fuzzy** — a partial name returns not-found, so there are no false positives. The discriminator is the `path` line, **never** the exit code: per `_shared-rules.md` §24 the CLI's exit status is unreliable in both directions.

...
4. **Route project artefacts and move the project file:**
   ...
   **⚠ How to move files — use the lock-aware Obsidian wrapper, never shell `mv`.** Every move in this step (artefacts *and* the project file) must go through the wrapper so concurrent writes serialize and inbound links auto-heal:

   ```bash
   SOURCE="{VAULT}/03 Projects/[Project Name].md"
   DESTINATION="{VAULT}/[shelf]/[Project Name].md"
   SOURCE_SHA256="$(python3 -c 'import hashlib,sys; print(hashlib.sha256(open(sys.argv[1], "rb").read()).hexdigest())' "$SOURCE")"
   ...
   **Step 4b — Move the project file:**
   ...
   - **Create the destination folder first** — `mkdir -p "{VAULT}/[shelf]"`, using the shelf **confirmed** at the top of Step 4. Only ever `mkdir` a path the user confirmed; never let this line be the thing that invents a new shelf. The wrapper requires the destination parent to exist.
   - Then hash and move the file from the exact path recorded in Step 2 using `locked-edit.sh --move` (see the move-mechanics note above)

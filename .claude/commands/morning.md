...
**Strategic Overview (render):** Read every `.md` in the `{VAULT}/03 Projects/` root (not `Cold/`, not `Backlog/` — folder location is project status: root = active, `Cold/` = paused). Each root doc must carry `bucket:` YAML frontmatter (values per the vault's bucket taxonomy — Project Doc Format in `07 System/Vault Organisation Principles.md`) plus a `## Current Objective` and `## Next Actions` section. If a doc lacks any of these, **fail closed for that doc** — list its missing fields in the dashboard rather than inventing content. Then render a terse dashboard to `{VAULT}/01 Now/Strategic Overview.md`, overwriting each run (Write tool is correct here — it's a regenerated view, not a shared planning file). The file opens with a header line stating it is a generated read-only view, regenerated by /morning and by any skill that changes the `03 Projects/` root, never edited by hand — project docs are SSOT. (Name the second writer by role, not by listing the skills: the list grows, and a header enumerating today's renderers goes false the moment another one is added.) Format: grouped by bucket; per project: name as a wikilink, Current Objective verbatim (1-2 lines), top 1-3 Next Actions, and any dated items falling in the next 7 days. Hard output cap ~60 lines. If the root doc count exceeds the **active project cap** (resolve it first: `grep -F '**Active project cap:'` over `{VAULT}/07 System/Vault Organisation Principles.md` → *Project Doc Format*, and state the value found. **`-F` is required** — a leading `**` is a repetition operator to some greps, which error out instead of matching. Exit 1, or a line yielding no number, means state `cap line unreadable — using default 5` and proceed on 5, so a failed read is never mistaken for a vault that states no cap. **Any other non-zero exit is a tool error, not an absent line** — report it and stop, rather than falling through to the default, which is the failure this branch exists to prevent) — flag it in the landscape output and ask which project moves to `Cold/`.

**This Week cap check** (resolve the caps first: `grep -F '**This Week cap:'` over `{VAULT}/07 System/Vault Organisation Principles.md` → *Project Doc Format*, and state the two values found — the `N/week` window cap and the `N/day` day cap. **`-F` is required** — a leading `**` is a repetition operator to some greps, which error out instead of matching. Exit 1, or a line that does not yield both values, means state `cap line unreadable — using defaults 30/week, 10/day` and proceed on those, so a failed read is never mistaken for a vault that states no cap. **Any other non-zero exit is a tool error, not an absent line** — report it and stop, rather than falling through to the defaults, which is the failure this branch exists to prevent)**:** count unchecked `- [ ]` items in the forward window (today + 6 days) of This Week.md. Over the window cap in total, or any single day over the day cap: flag it in the landscape with the counts and offer a triage (route overflow to project docs / Tickler / Whimsy) before planning today. Never silently add items to an over-cap window. Run the count **after** the Tickler→This Week migration below has landed — and if a migration itself would breach the cap, hold that item in the Tickler (bump its date) and flag it instead of migrating. **Never bump a deadline-bearing item past its window (§18)** — if the cap can't absorb it, flag the breach and ask which non-dated item drops instead.

Read and present:
- **Active projects:** From the Strategic Overview just rendered — project names, objectives, and next actions per bucket
...
- **Time-sensitive items:** Scan the Strategic Overview's dated items and recent sessions for deadlines, urgencies
...
- Project docs (the `## Next Actions` queues surfaced in the Strategic Overview)
...
- **May create/update:** This Week.md (weekly plan with day sections), Strategic Overview (rendered dashboard, overwritten each run)

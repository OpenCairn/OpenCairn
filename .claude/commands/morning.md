...
**This Week cap check (30/week, 10/day — default; the vault's Project Doc Format section wins if it states different caps):** count unchecked `- [ ]` items in the forward window (today + 6 days) of This Week.md. Over 30 total, or any single day over 10: flag it in the landscape with the counts and offer a triage (route overflow to project docs / Tickler / Whimsy) before planning today. Never silently add items to an over-cap window. Run the count **after** the Tickler→This Week migration below has landed — and if a migration itself would breach the cap, hold that item in the Tickler (bump its date) and flag it instead of migrating. **Never bump a deadline-bearing item past its window (§18)** — if the cap can't absorb it, flag the breach and ask which non-dated item drops instead.

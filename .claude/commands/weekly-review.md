...
2. **Check for Hygiene Report and gather the week's data:**
   ...
   **Week's activity data:**
   ...
   - Read the `03 Projects/` root docs to see active projects — each carries `bucket:` frontmatter plus `## Current Objective` and `## Next Actions`; folder location is the status (root = active, `Cold/` = paused, `Backlog/` = unstarted). If the root doc count (excluding `Cold/` and `Backlog/`) exceeds the **active project cap** — read it from `{VAULT}/07 System/Vault Organisation Principles.md` → *Project Doc Format*, defaulting to 5 if the vault states none — flag it and ask which project moves to `Cold/`

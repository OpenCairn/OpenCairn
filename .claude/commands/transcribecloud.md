...
**If the user picks the published transcript:** the body is already in `$WORK/body.md` from the **`_shared-rules.md` §15 extractor** (run during the validate step above). Do **not** enter Phase 8's `For each JSON transcript file` loop (there is no JSON here), and run no LLM cleanup pass — it is already human-edited. Confirm the output dir/filename here if Phase 1 step 7 didn't (its prompt assumes a cloud run; the episode title is a sensible default filename). Write the header from Phase 8 step 5's template, then **append `$WORK/body.md` via the shell** (`_shared-rules.md` §14 — header via editor, body via shell, no `Edit` afterward): **Source** = transcript URL, add an **Original media:** line for the YouTube/audio URL, **Model:** `published transcript (human-edited)`, mark **Diarisation** and **Cleanup** `n/a` (or drop them), keep **Duration** only if known. Don't print the whole body to the conversation. **Skip Phases 2–8's loop entirely — no pod is provisioned, no money spent.** Otherwise continue to Phase 2.

...
- **SSH info retrieval:** `runpodctl pod get <id>` returns IP, port, and full `ssh_command` directly — no manual web UI step needed for exposed-TCP SSH. The **proxy-SSH** command (`PODID-HASH@ssh.runpod.io`) comes from the REST API instead (`GET https://api.runpod.io/v2/pods/{id}` → `ssh.proxy.command`).

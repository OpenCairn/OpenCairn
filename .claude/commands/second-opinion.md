...
3b. **Grok** via Bash, in parallel with the other three. **Pass `timeout: 300000`** — xhigh reasoning can think >90 s before the first byte, so the default 120 s kills it. Invocation per the **canonical block in `_shared-rules.md` §10**:
...
4. **Invoke the reviewers in a single message with concurrent tool calls**, same as Phase 2A — parallel, not sequential:
   ...
   - **Gemini CLI via Bash** with `timeout: 300000`, run from the cwd recorded in step 3. The Phase 2A policy file lives in a different Bash call and its variable is gone, so **recreate the read-only policy in the same block** (per `_shared-rules.md` §10) rather than referencing a stale one — an empty `--policy` argument is accepted silently and enforces nothing. Substitute `<round2-path>` with the path from step 2, `<index>` with the session index from step 3, and `<root>` with the target's root:
     ```

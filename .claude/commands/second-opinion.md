...
> ⚠️ **Mode B is only partially validated.** The Codex resume leg has been exercised in anger (7 Jun 2026: round-2 `codex exec resume` round-tripped with prior context; found+fixed the flag-order bug — sandbox flags must precede `resume`, verified codex-cli 0.136.0). The **Claude SendMessage and Gemini `--resume` legs remain first-pass specification** derived from documentation, not observed behaviour — operational gaps may surface around Agent ID lifetime across session boundaries, SendMessage tool availability, and gemini session indexing under concurrent use. Treat the unvalidated legs as a specification you are helping to validate: when you hit a gap, surface it back to this skill file rather than working around it silently — don't pretend Mode B is production-ready where it hasn't been proven.
...
- **Mode B is only partially validated.** The Codex resume leg is proven in real use; the Claude SendMessage and Gemini `--resume` legs are untested specification — treat those with the scepticism you'd apply to any untested procedure. Surface gaps back to this file rather than working around them silently — the skill gets sharper with every real use.

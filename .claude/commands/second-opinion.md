...
**Shared rules:** read `_shared-rules.md` (core) and `_shared-rules-reviewer.md` §§10, 16, 20 and 23 from this command's installation directory before preparing the review. Other supplements load only when their operation is needed, per the core directory.
...
**Before launching: confirm the reviewers are available** — run `gemini --version`, `codex --version`, and `~/.claude/scripts/xai_client.py --probe` via Bash. If either CLI errors out, it isn't installed; drop it and note the reduced panel per step 4. The Grok probe has no binary to version-check — it exits 0 when the client resolves `XAI_API_KEY` from the process environment or `~/.claude/settings.json`, 1 otherwise; run it *before* despatch so an unavailable key degrades the panel up front rather than failing mid-run. If Gemini's version is older than 0.40 (no Policy Engine), use the write-capable fallback invocation in step 2 — but note that path has **no read-only enforcement** (the step-6 integrity guard is the *only* protection), so prefer upgrading to gemini ≥ 0.40 and treat findings from a 0.33–0.39 Gemini as lower-assurance.

Send the brief to all reviewers in a **single message** with concurrent tool calls:

1. **Fresh Claude** via the Agent tool. Use `subagent_type: general-purpose`. **Read the scratch file's contents and pass them verbatim as the Agent's `prompt` argument** — the same payload the Gemini and Codex calls receive. Do not paraphrase, summarise, or extract: the brief *payload* must be identical across seats, and seats may differ only in whether they read the target from source or from the inlined appendix. The transport wrappers (the Agent prompt frame, the CLI `-p` string) may differ but must add no substantive instructions, or you've re-introduced framing asymmetry. **The Grok seat needs the target inlined**, which the wrapper appends as a clearly delimited appendix plus manifest; send that same appended brief to every seat, and keep the read-from-source instruction for the three that can read — otherwise you either fork the brief (asymmetry) or destroy the read-list evidence for the CLI seats (worse). Tell the Agent explicitly in the prompt: independent judgement, disagree where warranted, no sycophancy, no "looks good overall" hedging. If you skip the disagree directive, fresh Claude defaults to polite confirmation of whatever framing you gave it.

   **After the tool call completes, surface the Agent ID in your user-visible response text** — e.g. `Panel spawned: Claude agent a5abc789520b0879a`. This makes the ID recoverable from the conversation transcript even after the raw tool result scrolls out of context, which matters for Mode B round 2. Don't rely on re-reading the tool result hours later.
...
3b. **Grok** via Bash, in parallel with the other three. **Pass `timeout: 300000`** — xhigh reasoning can think >90 s before the first byte, so the default 120 s kills it. Invocation per the **canonical block in `_shared-rules-reviewer.md` §10**:

   ```
   ~/.claude/scripts/xai_client.py --panel-review <brief> --source <target> [--source <target> ...] > <grok-out>
   ```
...
4. **Invoke the reviewers in a single message with concurrent tool calls**, same as Phase 2A — parallel, not sequential:
   ...
   - **Gemini CLI via Bash** with `timeout: 300000`, run from the cwd recorded in step 3. The Phase 2A policy file lives in a different Bash call and its variable is gone, so **recreate the read-only policy in the same block** (per `_shared-rules-reviewer.md` §10) rather than referencing a stale one — an empty `--policy` argument is accepted silently and enforces nothing. Substitute `<round2-path>` with the path from step 2, `<index>` with the session index from step 3, and `<root>` with the target's root:
     ```
   ...
   - **Grok via Bash** with `timeout: 300000` — **always replay, never resume** (the wrapper stores nothing, so there is no session handle). Re-send the round-2 prompt with the same `--source` list persisted in Phase 2A, plus the round-1 output so the seat carries its own prior findings:
     ```
     ~/.claude/scripts/xai_client.py --panel-review <round2-path> --source <target> [--source <round1-grok-out>] > <round2-grok-out>
     ```
...
3. **Resume the round-1 reviewers (Mode B, Phase 2B), in a single message with concurrent calls** — SendMessage to the Claude Agent, `gemini --resume`, `codex exec resume <UUID>` — each handed *its own* pool from step 1. No re-brief: each reviewer still has its round-1 context, and you're only adding the others' findings. **Grok has no session to resume**: replay it per Phase 2B step 4's Grok bullet, with its pool as the prompt and the persisted `--source` list, and tag its verdicts `[… xr, fresh-with-replay]`. If any other reviewer can't be resumed, run it fresh with its pool as the brief and tag it the same way.
...
- **Mode B is only partially validated.** The Codex resume and Gemini `--resume` legs are proven in real use; the Claude SendMessage leg is untested specification — treat it with the scepticism you'd apply to any untested procedure. Surface gaps back to this file rather than working around them silently — the skill gets sharper with every real use.

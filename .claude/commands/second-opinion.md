...
**Before launching: confirm the CLI reviewers are available** — run `gemini --version` (and/or `agy --version`, the Antigravity alternative for the Google seat — see step 2 and §10) and `codex --version` via Bash. If either errors out, that CLI isn't installed; drop it and note the reduced panel per step 4. If Gemini's version is older than 0.40 (no Policy Engine), use the write-capable fallback invocation in step 2 — but note that path has **no read-only enforcement** (the step-6 integrity guard is the *only* protection), so prefer upgrading to gemini ≥ 0.40 and treat findings from a 0.33–0.39 Gemini as lower-assurance.

Send the brief to all reviewers in a **single message** with concurrent tool calls:

1. **Fresh Claude** via the Agent tool. Use `subagent_type: general-purpose`. **Read the scratch file's contents and pass them verbatim as the Agent's `prompt` argument** — the same payload the Gemini and Codex calls receive. Do not paraphrase, summarise, or extract: the brief *payload* must be byte-identical across seats; the transport wrappers (the Agent prompt frame, the CLI `-p` string) may differ but must add no substantive instructions, or you've re-introduced framing asymmetry. Tell the Agent explicitly in the prompt: independent judgement, disagree where warranted, no sycophancy, no "looks good overall" hedging. If you skip the disagree directive, fresh Claude defaults to polite confirmation of whatever framing you gave it.
...
2. **Gemini CLI** via Bash, in parallel with the Agent call. **Pass `timeout: 300000` (5 minutes) to the Bash tool call** — the default 120 s will kill Gemini mid-review on any non-trivial target. This is load-bearing; the runner won't intuit it from "budget 5 minutes" alone.
   ...
   **After the call completes, capture and surface the Gemini session index.** Run `gemini --list-sessions | tail -3`; sessions are listed in ascending chronological order, so the **last/highest-numbered row** is the most recent. The leading numeric column (the `N.` before the prompt preview) is the index you want. Record it and surface it in the user-visible response text alongside the other handles (e.g. `Panel spawned: Claude agent a5abc789, Gemini session #12, Codex session 019e87a8-…`). You'll need this for Mode B round 2, and "latest" is not a safe shortcut (see Phase 2B step 2). Verified against gemini 0.38.2 and 0.40.1; if the output format ever diverges (no header, different column layout, reversed ordering), note what you saw and flag it back to this file — gemini's `--resume` only accepts an index number or the string `"latest"`, so if index parsing breaks, Mode B round 2 isn't recoverable and you fall back to a fresh Mode A panel.

   **Antigravity (`agy`) — the other Google-seat CLI.** Google retired the `gemini` CLI free and AI Pro/Ultra subscription tiers on 2026-06-18 in favour of Antigravity (`agy`); the panel supports either. If `agy` is installed the §10 probe prefers it, and you despatch via the **`agy` branch of §10** instead of the `gemini` lines above. Three differences from the Gemini CLI: (1) `agy` has **no read-only flag** (its `-p` mode auto-edits), so §10 isolates it in a throwaway git worktree rather than a `--policy` deny-file; (2) it **fails silently on quota** — `agy -p` exits 0 with empty stdout on a `RESOURCE_EXHAUSTED` 429, so treat empty/whitespace output as an **unavailable seat** (step 4), never a clean review; (3) its session handle is a **conversation UUID** read from `~/.gemini/antigravity-cli/cache/last_conversations.json` (a `cwd → uuid` dict), not a list index. The `timeout: 300000`, the integrity backstop (step 6), and the parallel-despatch shape are identical. `agy` also has no API-key env var (auth is interactive OAuth in the keyring); the gemini CLI, by contrast, can run on an AI Studio API key. Full agy mechanics — the worktree snapshot, the ~270s watchdog, the empty-output fallback — live in §10.
...
4. **Invoke the reviewers in a single message with concurrent tool calls**, same as Phase 2A — parallel, not sequential:
   ...
   - **Gemini CLI via Bash** with `timeout: 300000` — substitute `<round2-path>` with the path recorded in step 2:
     ```
     cat <round2-path> | gemini -p "Continue the prior review." --resume 12 --policy "$RO_POLICY" -o text   # recreate $RO_POLICY per _shared-rules.md §10 if the temp file is gone
     ```
   ...
   - **Antigravity (`agy`) via Bash** with `timeout: 300000`, **unsandboxed** — if round 1 used the agy seat, it **always uses fresh-with-replay** (agy true-resume works same-cwd only and §10 tears the worktree down — see the Phase 2B warning): re-run the **§10 `agy` branch verbatim** (its live-tree worktree snapshot, watchdog, traps, empty-output guard) with a round-2 brief that quotes the round-1 findings verbatim. Do **not** hand-roll a bare `worktree add … HEAD` — that reads the stale committed tree and skips the guards. Tag `[Gemini r2, fresh-with-replay]`.
...
Once the reports come back, write the synthesis. **Tag every finding with attribution** — name the reviewers who raised it: `[Claude]`, `[Gemini]`, or `[Codex]` for unique finds; the combination for shared ones (`[Claude+Gemini]`, `[Gemini+Codex]`, …), with `[All]` reserved for findings every reviewer on the panel raised; append `r2` for Mode B round-2 points (`[Claude r2]`, `[All r2]`), and `xr` for cross-examination verdicts from the Phase 3.5 peer-review matrix, naming raiser then cross-judges (`[F3: Gemini, Claude+Codex xr]`). Attribution is not optional: without it, the panel's provenance is lost and the tiebreak becomes unreviewable. The user can't audit your reasoning if they can't see which reviewer raised what, or which round it came from.
...
3. **Uniquely flagged (`[Claude]`, `[Gemini]`, or `[Codex]`)** — issues only one reviewer raised. Judge each on merit: a real issue, or a reviewer idiosyncrasy? Legitimate misses get promoted to the action list; shallow observations get dropped with a reason.
...
3. **Resume the round-1 reviewers (Mode B, Phase 2B), in a single message with concurrent calls** — SendMessage to the Claude Agent, `gemini --resume`, `codex exec resume <UUID>` — each handed *its own* pool from step 1. No re-brief: each reviewer still has its round-1 context, and you're only adding the others' findings. If a reviewer can't be resumed, run it fresh with its pool as the brief and tag `[… xr, fresh-with-replay]`.
...
- **Attribution is mandatory.** Every finding in the synthesis names the reviewers who raised it — `[Claude]`, `[Gemini]`, `[Codex]`, a combination, `[All]`, or any of these with an `r2` (round-2) or `xr` (cross-examination) suffix. This lets the user audit your tiebreak and catches the laundering failure mode where reviewer claims get presented as your own conclusions.

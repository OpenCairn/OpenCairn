...
5. **Snapshot the preimage of every file hashed in 3 and 4**, exactly as `/provenance` Step 5 does for work products — same `07 System/.Provenance/<date>-<name>-<short8>.snapshot<ext>` naming, same `[[ -e ]] ||` guard so a re-run never overwrites one. Session logs and transcripts were previously hashed *without* a preimage copy, on the assumption that a finalised log stops changing. It does not: a log is an indexed markdown file carrying wikilinks, so renaming or moving **any** note it links to rewrites it in place, indefinitely and long after the hash. Without a snapshot the attested bytes then survive only in version-control history, which fails for anything predating the vault's own history. The snapshot is what makes the attestation self-contained.

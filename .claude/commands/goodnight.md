...
**Cap awareness** (resolve the caps first: grep `{VAULT}/07 System/Vault Organisation Principles.md` → *Project Doc Format* for a line starting `**This Week cap:` and state the two values found — the `N/week` window cap and the `N/day` day cap; if that line is absent, state `cap line not found — using defaults 30/week, 10/day` and proceed on those, so a failed read is never mistaken for a vault that states no cap)**:** before moving items into future day sections, count the destination day's unchecked items and the forward-window total (today + 6 days). Day at the day cap or window at the window cap → route to the Tickler or the project doc instead; §18 still binds — a deadline-bearing item takes the Tickler, never the project doc or Whimsy. Never silently stack an over-cap surface.

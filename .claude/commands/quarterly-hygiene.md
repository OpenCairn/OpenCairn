...
**Scoped rule loading:** `_shared-rules-planning.md` before routing; `_shared-rules-reviewer.md` before reviewer/model checks; `_shared-rules-content.md` before structural operations. Read the applicable numbered sections at that point, from the same directory as the core `_shared-rules.md`; do not preload unrelated supplements.
...
7. **Skill-library flywheel audit (DRAFT SPEC — heuristics unvalidated; propose, never auto-apply; optional — skip freely on first runs or when time-boxed, noting "flywheel audit skipped" in the report).**
   ...
   - **Inventory** first-level user-authored files in the commands directory (`~/.claude/commands/*.md` or `{VAULT}/.claude/commands/*.md`). State this scope in the report. Derive the search targets rather than working a fixed list — a hardcoded list drifts into naming only mechanisms already indexed, which makes the "unindexed" branch unreachable and the audit a no-op:

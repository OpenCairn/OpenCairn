...
- **Defaults skip files `grep -r` searches, and the skip is not predictable by inspection.** `rg` omits dot-prefixed entries *and* honours `.gitignore`/`.ignore`, and the two interact: a negation (`!*.md`, `!*/`) re-admits hidden paths `rg` would otherwise skip, while an ignore rule excludes plainly visible ones. Reach is the combination, so reading either mechanism alone mispredicts it in **both** directions — the same command can reach one dot-directory and silently miss its sibling. The failure is always the same shape: an unsearched tree reporting as a clean negative. **Prove reach; never reason about it.** Plant a match at the most obscure path the search must cover and confirm the command reports it, or compare once against `grep -r`. `--hidden --no-ignore` forces full reach. Until that check has been run, an `rg` null over such a tree is a statement about your flags, not about the tree.

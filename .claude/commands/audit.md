...
6. **Record the run** once the terminal state is reported, panel mode only, whatever the seat count. Write the Phase 3 synthesis and this terminal-state report to `<run-dir>/synthesis.md` with the Write tool (the coder needs the dispositions, and the script warns when it is missing). Resolve the vault (`resolve-vault.sh`, §1), then:
   ```bash

...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Welcome back[, name from the loaded AGENTS.md guidance — omit if unknown].
...
7. **Update based on the user's answers:**
   - Record completed loops in the awaken summary's "Completed during break" lines (step 8); flip any matching `- [ ]` items in the SSOT files (This Week.md, Tickler, project docs) to `[x]` — write mechanism per `_shared-rules.md` §5 (`locked-edit.sh`, never a raw edit; `write-tickler.sh` for Tickler). The snapshot itself keeps plain bullets — there are no checkboxes to flip there, and it stays a frozen record.
...
```
   ...
   Render the block above, then append it through `"{VAULT}/.claude/scripts/locked-edit.sh" "<snapshot path>" --append`; never redirect or edit the snapshot directly. Once the summary is appended: completed or dropped projects → route through `$complete-project` (it owns artefact routing and the link-safe move out of the `03 Projects/` root — don't improvise a move here). It runs after the write so an abort there leaves the orientation record intact.
...
10. **Update project docs** (via `locked-edit.sh`, never a raw edit — project docs are shared planning files, see `_shared-rules.md` §5):
...
Continuations:
- **Option 2:** read the `03 Projects/` root docs and the most recent session logs, present them in place of the step 4 summary, then run steps 5-11 against that state. Skip the snapshot append in step 8; keep the orientation summary in the response and let the next `$park` capture it in the current session log.

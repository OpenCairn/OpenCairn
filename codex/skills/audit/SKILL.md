...
**Default (no flag): panel.** Run all available reviewers — Claude + Gemini + Codex + Grok, one seat per family — each performing Phase 2 independently, then synthesise per Phase 3. Graceful degradation is the rule: probe availability first and run with whoever's present. The first three read from source; Grok is an API seat that reviews inlined sources (see `_shared-rules-reviewer.md` §10) — it is a full seat with a different evidence standard, not a lesser one.
...
Compact panel despatch (single message, concurrent calls). The canonical command block — read-only policy file, Gemini/Codex invocations, out-of-cwd workspace flags — lives in **`_shared-rules-reviewer.md` §10**; use it verbatim rather than reciting from memory (it is the single source of truth; this file deliberately carries no copy to drift). Audit-specific notes on top of it: every seat (Claude included) is a CLI/API despatch — pipe the brief file to each and give each call a 25-minute ceiling. Under Codex, use §10's verified yield-and-poll mechanism so a seat can outlive `exec_command`'s initial 10-second window. Despatch from the target's root: Gemini's reads are sandboxed to the **cwd it launches from** (not `~` — verified, see §10) and Codex reads relative to its workdir, so a target outside the despatch cwd needs §10's `--include-directories <root>` / `-C <root>` flags.

**Every seat's output lands in the run directory §10 creates** (`$RUN_DIR`, one file set per seat: `.out`, `.err`, `.time`, plus `claude.json`). Print the path in visible text and paste it literally into every despatch call; those files, not this transcript, are the per-seat record the run leaves behind.
...
**Integrity guard (same as `second-opinion/SKILL.md` Phase 2A step 6).** The §10 `--policy` read-only file makes Gemini's edit tools "not found" (a hard guarantee on gemini 0.40.x), but keep this as a cheap backstop — it confirms the policy loaded and is the only protection on the no-`--policy` fallback. It depends on the pre-flight baseline taken above. After the panel returns and before Phase 3, re-hash the targets against the baseline; restore a change a reviewer reports making from the baseline copy (never `git checkout -- <file>`, which also discards pre-existing working-tree edits), leave an unattributed change in place and surface its diff for the user, and compare any "proposes text that already exists" finding against the baseline copy, not the working tree.
...
6. **Record the run** once the terminal state is reported, panel mode only, whatever the seat count. Write the Phase 3 synthesis and this terminal-state report to `<run-dir>/synthesis.md` with the Write tool (the coder needs the dispositions, and the script warns when it is missing). Resolve the vault (`resolve-vault.sh`, §1) and gate the archive layout — `"{VAULT}/.claude/scripts/check-archive-layout.sh" --enforce "{VAULT}"`; on error record without `--archive` and say so, since the run directory would land in a pre-migration archive — then:
   ```bash
   "{VAULT}/.claude/scripts/panel-run-record.sh" "<run dir>" --skill audit --round <n> --target "<what was audited>" --claude-model <model id from claude.json, when that seat ran> --vault "{VAULT}" --archive
   ```

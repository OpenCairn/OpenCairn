...
The panel despatch, attribution tags, and no-vote-counting tiebreak are `$second-opinion`'s machinery — this skill reuses it rather than reinventing it (see the *Parallel cross-model panel despatch* pattern in `_shared-patterns.md`). Read `second-opinion/SKILL.md` if you need the full despatch mechanics.

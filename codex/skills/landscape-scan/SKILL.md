...
**⛔ Read-the-source gate (mandatory).** Any *comparative claim that names a specific local skill* — that an external finding obsoletes / upgrades / replaces / collides with / is inferior or superior to / makes redundant `$X` — MUST be backed by reading `$X`'s own source *this run*, **wherever the claim appears** (Step 7's fit pass, Step 8, or any section of the report). Reviewer seats and external write-ups only ever see a name plus a one-line description, so their comparative claims are *structurally speculation*. **Name/description overlap is a prompt to open the file, never a verdict.**

- **Who reads:** the actor that writes the claim. A reviewer seat that can't load local skills emits "name overlap — source unread" and leaves the verdict to the orchestrator, which opens the named source itself before accepting or reporting it.
- **What to read:** the named Codex skill at `$SKILLS_ROOT/<name>/SKILL.md`. If it has not been ported, read the Claude source at `~/.claude/commands/<name>.md` and label that source explicitly. Read enough of the body to establish its operating layer and mechanism, not just the frontmatter one-liner.
...
1. `${CODEX_HOME:-$HOME/.codex}/skills`;
2. `$HOME/.agents/skills`;
3. the nearest repository `.agents/skills` directory visible from the current working directory;
4. `~/repos/OpenCairn/codex/skills` for a canonical development checkout.
...
- A topic word in the triggering message or arguments (e.g. `$landscape-scan cybersec`, "run a cybersec landscape scan") → that topic.
- No topic word → default topic **`ai-cc-pkm`** (preserves the original AI/Claude Code/PKM behaviour exactly).

Resolve the profile file at `$SKILL_DIR/landscape-profiles/<topic>.md`. If the named profile doesn't exist, list the available profiles (`ls "$SKILL_DIR/landscape-profiles/"`) and ask the user which to run — don't silently fall back to default when a topic was explicitly named.
...
- **URLs present AND a scan verb ("scan" / "landscape scan" / "run landscape scan")** → both modes. The skill invocation itself is **not** a scan verb — `$landscape-scan <topic>` plus a URL pile and nothing else is digest mode, since the bare-invocation rule above is scoped to runs with no URLs. The verb has to appear in the natural-language request.
...
If the vault contains a capability-audit project doc, update the relevant domain section. Locate it rather than assuming a filename — `find "{VAULT}/03 Projects" -maxdepth 1 -type f -iname '*capability audit*.md' -print` — and if nothing matches, say so instead of creating one. Write it through `locked-edit.sh` per `_shared-rules.md` §5.
...
Draft the report outside the vault. Install `{VAULT}/06 Archive/Landscape Scans/YYYY-Www<suffix>.md` through `locked-edit.sh`: use `--append` for a missing file. If it already exists (a second run this week), append a dated addendum through `--append` rather than overwriting, and never invent a suffixed filename (`…b.md`) — the profiles' prior-scan globs won't match it. The engine supplies the common wrapper; the profile supplies the middle finding-sections:
...
## Unfetchable
- [URLs that failed every rung of the §26 fetch ladder, with reason; or "None this run."]
...
- [Any edits made to AGENTS.md, context files, skills, profile files, tasks added to This Week / project hubs, or config — anything the scan surfaced that resulted in immediate in-session action. Makes the report self-contained. If none: "None."]
```

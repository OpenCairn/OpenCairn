...
- **Cap check before adding** — count target surface first; overflow → ask which drops. → `morning`/`weekly-review`/`weekly-hygiene`/`start-project` (active project cap). **A real cap, and the only one:** a project root over its cap is a commitment problem the user resolves by moving something to `Cold/`, so the ask is the point
- **Advisory threshold, reported not enforced** — count the surface, report the figure against its threshold, change nothing else. → `morning`/`goodnight`/`weekly-review` (This Week load), `morning` (Working Memory overflow), `weekly-hygiene` (scratchpad line count). Distinct from the cap above and easily collapsed back into it: the tell is a threshold that starts redirecting items, holding migrations, or asking which drops. A dated item belongs on its date whatever the count says, so enforcing here strands work in the Tickler while the day section reads empty — and the decision to strand it has no durable surface, so the next run re-derives it and re-asks. Enforce a cap where the overflow is a *choice* the user is making; report a threshold where it is a *fact* about what is due. **Open transfer:** `weekly-hygiene`'s flag carries a first-flagged marker preserved verbatim across refreshes, so a long-standing breach is distinguishable from a new one — the other threshold sites have no such memory and read identically on day 1 and day 40

...
- `_shared-rules.md` holds core runtime rules and routes to planning, reviewer and content supplements; commands load the relevant sections when needed.
- This file holds *where to find the battle-tested version of a pattern* you'd want when building or improving a skill.

**Consult it** whenever you build or substantially edit a skill: scan for patterns this skill wants, then **read the named reference skill for the real implementation** and adapt it. Cross-pollination is the point — skills get sharper by sharing infrastructure. In Claude Code, `/setup-hooks` can enable a Stop-hook reminder; in Codex, consult this index directly.
...
- **Progress reporting** — `[i/N]` + elapsed per item; heartbeat on long loops. → `transcribecloud` (Phase 5 in the Claude source), `ocr` capture loop
- **Cost/time estimation up front** — project units × cost; confirm before spend. → `transcribecloud`
- **Parallel cross-model panel despatch** — one seat per model family, identical brief, concurrent. → `second-opinion` (command block: `_shared-rules-reviewer.md §10`)
- **Seat tiering — judgement floats, verification may pin** — session model default; optional strong floor with reason. → `_shared-rules-reviewer.md §10`
- **Reviewer evidence attestation** — read-list / command / manifest / URL+quote; class outranks votes. → `_shared-rules-reviewer.md §23`
- **Out-of-band evidence in reviewer briefs** — embed every source verbatim; omissions read as fabrication. → `_shared-rules-reviewer.md §16`
...
- **Page text outranks transcript body** — human-written show notes win names + speakers. → `_shared-rules-content.md §15`
...
- **Web-fetch fallback ladder** — free static extractor first; metered scrapers last. → `_shared-rules-content.md §26`
...
- **Cross-call scratch path: derive or record, never remember** — stable-input slug; else `mktemp` once and record it. → `_shared-rules-content.md §15`, `podcast-digest` Phase 0 (derived), `second-opinion` Phase 1 (recorded), `quarterly-hygiene` Step 6 (recorded + hash)
...
- **Link-aware moves: never raw `mv`, batches are fine** — CLI behaviour + verification live in one place, not in each skill. → `_shared-rules-content.md §24`, `quarterly-hygiene` Step 6, `complete-project` Step 4, `inbox-processor` Step 4
...
- **Transcript export → `--days 7 --all-projects`** — both project + mtime-window axes clobber the date-canonical day file. → `goodnight` C1.h + Step 16, `park` Step 11, `weekly-hygiene`
- **Preference quiz with ranked hard requirements** — interactive quiz in conversation (numbered questions, concrete options); skip context-answered; rank firmest→negotiable. → `shop` Phase 2, `book-stay` Step 2
...
- **Auto-save git is not pre-state** — commit boundaries misread prior *content*; verify per-commit. Two sides: forbid it in the reviewer's brief (→ `park` Step 9, `goodnight` Step 15(c), `goodnight` C1.g) and re-check any git-derived finding before accepting it (→ `park` Step 9, `goodnight` Step 15(e))
- **Session-boundary attribution** — brief's file list bounds *authorship*; commit window doesn't. → `_shared-rules-reviewer.md §20`, `park` Step 9, `goodnight` Step 15(c)
- **Value provenance check (SOURCE)** — written values trace to user, tool, or tag. → `_shared-rules.md §19`, `park` Step 2(c), `goodnight` Step 14b, `_shared-rules-reviewer.md §16` (brief evidence: primary/secondary/unverified)
...
- **Deadline token forces dated surface** — deadline-bearing items route to dated target, never undated doc. → `_shared-rules-planning.md §18`, `park` Step 7, `goodnight` Step 9, `weekly-review` Step 4
...
- **Confirmatory-only checks pass under both hypotheses** — name the observation that actually differs. → `ocr` (last-frame message identity), `_shared-rules-content.md §15`, `_shared-rules.md §25` (a control sharing the blind spot agrees either way)
...
- **Cap check before adding** — count target surface first; overflow → ask which drops. → `morning`/`weekly-review`/`weekly-hygiene`/`start-project`/`set-project-status` (active project cap). **A real cap, and the only one:** a project root over its cap is a commitment problem the user resolves by moving something to `Cold/`, so the ask is the point
- **Advisory threshold, reported not enforced** — count the surface, report the figure against its threshold, change nothing else. → `morning` (Working Memory overflow), `weekly-hygiene` (scratchpad line count). Distinct from the cap above and easily collapsed back into it: the tell is a threshold that starts redirecting items, holding migrations, or asking which drops. Enforce a cap where the overflow is a *choice* the user is making; report a threshold where it is a *fact* about the surface. A threshold nobody acts on is maintenance surface with no customer — delete it rather than tune it (the This Week load thresholds went this way). **Open transfer:** `weekly-hygiene`'s flag carries a first-flagged marker preserved verbatim across refreshes, so a long-standing breach is distinguishable from a new one — the other threshold sites have no such memory and read identically on day 1 and day 40
- **Veto-of-proposed-routes** — propose a destination per item; user pass = veto, not generate; unvetoed executes. → `migrate` component 3
...
- **Default-with-SSOT-deference** — resolve the value from the owning vault doc *first* (grep a named line prefix, state what was found), falling back to a stated default announced as `line not found — using default N`, so a failed read is never mistaken for a doc that states no value. → `morning`/`weekly-review`/`weekly-hygiene`/`start-project`/`set-project-status` (active project cap); owner: Vault Organisation Principles → Project Doc Format. Prose-only variant (states a default + "the vault wins if it differs", no resolve step) → `complete-project` (archiving convention); weaker, because nothing instructs the read — the hardcoded default is what actually gets used

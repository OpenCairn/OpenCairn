...
**Shared rules:** read `_shared-rules.md` (core) and `_shared-rules-reviewer.md` §§10, 16, 20 and 23 from the shared skills directory above this SKILL.md before preparing the review. Other supplements load only when their operation is needed, per the core directory.
...
**Mode A — Independent panel (parallel, fresh reviewers).** Reviewers from different model families — Claude, Gemini, Codex, and Grok — each seeing the work for the first time, running concurrently. Maximum non-anchoring; cross-model signal on agreement vs disagreement. Use when the question is "is my understanding of this right?" and you want judgement uncorrelated with the in-session conversation. This is the default mode and the one validated in real use. If a reviewer is unavailable, the panel runs with whoever's present (see Phase 2A step 4) — four families is the target, not a hard requirement.

**Mode B — Iterative deepening (sequential, resumed reviewers) — partially validated (Codex + Gemini legs proven in use; Claude leg's resume mechanics verified, not yet exercised in a full round 2), see Phase 2B warning.** Bring the *same* reviewer(s) back for round 2, 3, or more. They carry their prior context forward — you don't have to re-brief them on the work or the history. Use when:
...
**Optional: anonymized cross-examination.** Between the round-1 panel and your synthesis, you can run one cross-review pass — feed each reviewer the *other* reviewers' findings with **model identity stripped**, and ask them to challenge, confirm, or rank each one on its merits. Anonymization is the point: a reviewer should engage with the substance, not defer to "the Codex finding" or dismiss "the Gemini one." This surfaces which round-1 claims survive scrutiny by a different model family and which were one-model idiosyncrasies. It runs cheaply by *resuming* each round-1 reviewer and showing it only the findings it **didn't** author — self-recognition can't happen because a reviewer never sees its own findings, so no re-brief is needed. The result is a peer-review matrix: each finding judged by the reviewers who didn't raise it. See Phase 3.5. **Guardrail:** cross-review can manufacture false consensus — a reviewer may cave to a confidently-worded peer claim that's actually wrong. Treat surviving findings as *better-tested*, not *confirmed*; the pass feeds your tiebreak, it never replaces it with a vote count.
...
4. **Write a self-contained brief** that each reviewer can read. For Mode A they read it cold; for Mode B they read it on top of their existing context. Include:
   ...
   - **Out-of-band evidence** — every source the work product's claims rest on that the reviewer cannot reach from the work itself, embedded verbatim under an established-fact heading, per **`_shared-rules-reviewer.md` §16** (the single source of truth; no copy here, to avoid drift).
   - The specific question(s) you want judgement on.
   - The output format: structured sections, word cap, explicit "no hedging, disagree where warranted, no sycophancy" directive, and **the evidence attestation each seat owes, per `_shared-rules-reviewer.md` §23** (it is the single source of truth; this file deliberately carries no copy to drift). Paste §23's attestation table into the brief body verbatim rather than cross-referencing it — the reviewer cannot read this directory. §23 carries the receipt enforcement, the spot-check-before-promoting rule, and the evidence-class tiebreak.
   - **A "what the work gets right" section in the output format, capped at ~5 bullets.** Without this, a reviewer who only reports problems leaves the synthesis unable to distinguish "section X was approved" from "section X was not mentioned" — and silence is not endorsement. Forcing reviewers to name what they think works also makes disagreement more visible in Phase 3: if Claude lists something as a strength and Gemini lists the same thing as a problem, that's a load-bearing disagreement you'd otherwise miss.
...
1. **Fresh Claude** via the `claude` CLI in print mode — `cat <brief> | claude -p "Follow the instructions in the piped input exactly."` — configured read-only per §10's despatch block (`--output-format json --disallowedTools "Bash,Write,Edit,NotebookEdit"` — verified: no write path, and a spawned sub-agent inherits the restriction). It receives the same payload as the Gemini and Codex calls. Do not paraphrase, summarise, or extract: the brief *payload* must be identical across seats, and seats may differ only in whether they read the target from source or from the inlined appendix. The transport wrappers (the per-CLI `-p` strings) may differ but must add no substantive instructions, or you've re-introduced framing asymmetry. **The Grok seat needs the target inlined**, which the wrapper appends as a clearly delimited appendix plus manifest; send that same appended brief to every seat, and keep the read-from-source instruction for the three that can read — otherwise you either fork the brief (asymmetry) or destroy the read-list evidence for the CLI seats (worse). The brief carries the disagree directive: independent judgement, disagree where warranted, no sycophancy, no "looks good overall" hedging — without it a fresh reviewer defaults to polite confirmation of whatever framing you gave it.
...
2. **Gemini CLI**, in parallel with the Claude call. **Give the call a five-minute ceiling** and, under Codex, use §10's verified yield-and-poll mechanism so the review can outlive `exec_command`'s initial 10-second window.
   ...
   Preferred invocation (gemini ≥ 0.40, which has the Policy Engine): use the **canonical despatch block in `_shared-rules-reviewer.md` §10** — the single source of truth for the read-only policy file, the piped `gemini` call, and the workspace flags for out-of-cwd targets — substituting `<brief>` with the path recorded in Phase 1. Why a `--policy` deny-file: it loads Gemini's **Policy Engine** (the non-deprecated successor to `--allowed-tools`) and a `deny` rule strips the named tools from the model's toolset entirely — **verified on gemini 0.40.x** as a *hard* read-only guarantee (the reviewer reports the edit tools "not found" while every read tool stays available). This **replaces `--approval-mode plan`**, which was *not* read-only (plan blocked shell but still exposed the `replace` edit tool, so a reviewer briefed to propose fixes wrote them straight into the target — the contamination that motivated this change). Further operational caveats — portable `mktemp` form, no `~/.gemini/policies/` drop, the post-run denied-tools check, untrusted-directory refusals — live in §10; read it rather than reciting from memory. The integrity guard in step 6 stays as a cheap backstop (and confirms the policy loaded). Gemini also has a real `-s/--sandbox`, but it needs a container runtime, so don't mandate it.

   Fallback if the flags aren't supported by the installed version (write-capable — the step-6 integrity guard is the only protection on this path):
   ```
   cat <brief> | gemini -p "Follow the instructions in the piped input exactly."
   ```

   **After the call completes, capture and surface the Gemini session index.** Run `gemini --list-sessions | tail -3`; sessions are listed in ascending chronological order, so the **last/highest-numbered row** is the most recent. The leading numeric column (the `N.` before the prompt preview) is the index you want. **Indices are per-project, scoped to the cwd the command runs from** — the same session carries a different number (or is absent entirely) when listed from another directory. Run the listing from the cwd you despatched from, and **record that cwd alongside the index**; Phase 2B's `--resume` must run from the same one. Each row also carries a per-session UUID in brackets — record it too, as it re-locates the row if indices shift, even though `--resume` won't accept it. Surface the index in the user-visible response text alongside the other handles (e.g. `Panel spawned: Claude agent a5abc789, Gemini session #12, Codex session 019e87a8-…`). You'll need this for Mode B round 2, and "latest" is not a safe shortcut (see Phase 2B step 2). Verified against gemini 0.38.2 and 0.40.1; if the output format ever diverges (no header, different column layout, reversed ordering), note what you saw and flag it back to this file — gemini's `--resume` only accepts an index number or the string `"latest"`, so if index parsing breaks, Mode B round 2 isn't recoverable and you fall back to a fresh Mode A panel.

3. **Codex CLI**, in parallel with the Claude and Gemini calls. **Give the call a five-minute window** — same reason as Gemini.

   Invocation: the codex line of the **canonical despatch block in `_shared-rules-reviewer.md` §10**, substituting `<brief>` with the path recorded in Phase 1. Rationale: `--sandbox read-only` gives Codex read-only tool access (it can Read files but not edit them), matching the independent-review intent — the analogue of Gemini's `--policy` deny-file (step 2); `--skip-git-repo-check` lets it run outside a git repository (the target often isn't one); the trailing `-` reads the brief from stdin. Output is plain text and pipe-friendly; a benign `failed to refresh available models` line may appear on stderr under flaky networks and can be ignored.
...
5. **A passing `--version` doesn't prove a reviewer can authenticate.** A CLI whose auth comes from an API key in an environment variable can report its version yet still fail the actual call — the key must be present in the shell the harness actually runs (Codex invokes `/bin/bash -lc`, which loads the profile; other setups may not, so don't assume a specific file). If a reviewer fails on **auth** rather than launch, set its key where the CLI reads it itself regardless of how the shell was started — the CLI's own config/env file is the portable fix across Linux, macOS and Windows (e.g. Gemini reads `~/.gemini/.env` containing `GEMINI_API_KEY=<key>`). This is distinct from step 4's "not installed": the binary runs fine, the credential just isn't reaching it.

6. **Integrity guard — confirm the panel didn't mutate the target.** With the `--policy` read-only file (step 2) Gemini's edit tools are stripped, so this is now a cheap backstop rather than the primary defence — but keep running it: it confirms the policy loaded, and it's the *only* protection on the no-`--policy` fallback path and for any reviewer whose sandbox leaks. Verify after the panel returns and before synthesising: re-hash every target and compare with the pre-despatch baseline. An unchanged target is clean. A changed target is either a reviewer write or a concurrent writer (another session, the user), and bytes alone cannot tell which — so attribute before touching it. If a reviewer's own output reports an edit attempt on that path, restore that file from the baseline copy (`cp` it back and confirm the hash matches) — **never `git checkout -- <file>`**, which also discards edits that were already in the working tree before the panel. If no reviewer reports an edit, the change is unattributed: leave it, show the baseline-to-current diff, and hand it to the user to reconcile. The contamination is also a synthesis trap: a finding that reads as "proposes text that already exists" usually means the reviewer already applied its own edit, so compare claims against the baseline copy, not the live working tree, before dismissing them. Note every restored or unattributed change in the synthesis.

### Phase 2B: Resume prior reviewers (Mode B)

> ⚠️ **Mode B is only partially validated.** The Codex resume leg has been exercised in anger (7 Jun 2026: round-2 `codex exec resume` round-tripped with prior context; found+fixed the flag-order bug — sandbox flags must precede `resume`, verified codex-cli 0.136.0). The Gemini leg was exercised 12 Jun 2026 (gemini 0.40.1): `gemini --resume <index> -p "<follow-up>" --policy <ro-file> -o text` round-tripped with prior context — the resumed session completed the remaining layers of an audit it had started, read-only policy holding. Caveat: that run resumed by *list index* minutes later with no concurrent gemini sessions; index stability under concurrent use is still unobserved — and while `--list-sessions` prints a per-session UUID, `--resume` accepts only an index or `latest`, so the UUID re-locates the row but can't be resumed directly: record it alongside the index, and under concurrent gemini use match the session by UUID/timestamp before resuming by its current index, or fall back to a fresh Mode A panel. The **Claude leg's resume mechanics are verified** — `claude -p --resume <session_id>` round-trips prior context; capture `.session_id` from `--output-format json`; re-pass the read-only flags on the resume call — but it has not yet carried a full review round 2, and resume across machine or version changes is unobserved. Treat the unvalidated leg as a specification you are helping to validate: when you hit a gap, surface it back to this skill file rather than working around it silently — don't pretend Mode B is production-ready where it hasn't been proven.

Use this phase instead of 2A when you're iterating — the reviewers are already spun up from a previous round.

0. **The Claude leg resumes via the CLI.** Under Codex there is no in-harness handle to a Claude reviewer; round 2 goes back through `claude -p --resume <session-id>` using the id captured in Phase 2A (re-pass `--output-format json` and the read-only `--disallowedTools` set on the resume call). If no session id was captured, the Claude leg isn't resumable; run it fresh with the round-1 output quoted in the brief and tag `[Claude r2, fresh-with-replay]`.

1. **Check the Claude session id is still on hand.** Mode B's Claude leg needs the session id captured in Phase 2A; `claude` sessions persist on disk, but resume behaviour across machine or version changes is unverified. If you can't find the id in the conversation transcript, run the Claude leg fresh-with-replay rather than guessing at a session.
...
3. **Look up the Gemini session index deterministically.** If you surfaced the index in Phase 2A (as instructed), use that — and run both the lookup and the `--resume` from the **cwd recorded with it**, since indices are per-project and renumber (or vanish) from a different directory. If not, run `gemini --list-sessions` from that cwd and identify the right session by UUID, timestamp and prompt preview. **Do not use `--resume latest`** — "latest" resolves to the most recent gemini session for the project, so if you or any other process has run gemini for anything in between (even a quick one-shot), `latest` resumes the wrong session. Always resume by numeric index.

4. **Invoke the reviewers in a single message with concurrent tool calls**, same as Phase 2A — parallel, not sequential:

   - **Claude CLI resume**: `cat <round2-path> | claude -p --resume <session-id> --output-format json --disallowedTools "Bash,Write,Edit,NotebookEdit"` — the resumed session keeps its prior context; you do **not** re-brief from scratch. If resume fails to round-trip, fall back to fresh-with-replay and tag `[Claude r2, fresh-with-replay]`.

   - **Gemini CLI** with a five-minute window, run from the cwd recorded in step 3. The Phase 2A policy file lives in a different shell call and its variable is gone, so **recreate the read-only policy in the same block** (per `_shared-rules-reviewer.md` §10) rather than referencing a stale one — an empty `--policy` argument is accepted silently and enforces nothing. Substitute `<round2-path>` with the path from step 2, `<index>` with the session index from step 3, and `<root>` with the target's root:
     ```
...
Once the reports come back, write the synthesis. **Tag every finding with attribution** — name the reviewers who raised it: `[Claude]`, `[Gemini]`, `[Codex]`, or `[Grok]` for unique finds; the combination for shared ones (`[Claude+Gemini]`, `[Gemini+Codex]`, …), with `[All]` reserved for findings every reviewer on the panel raised; append `r2` for Mode B round-2 points (`[Claude r2]`, `[All r2]`), and `xr` for cross-examination verdicts from the Phase 3.5 peer-review matrix, naming raiser then cross-judges (`[F3: Gemini, Claude+Codex xr]`). Attribution is not optional: without it, the panel's provenance is lost and the tiebreak becomes unreviewable. The user can't audit your reasoning if they can't see which reviewer raised what, or which round it came from.

1. **Confirmed by multiple reviewers (`[All]`, `[Claude+Gemini]`, …)** — findings more than one reviewer agrees on; the more families that converge, the higher the confidence. Still *not* ground truth: correlated agreement is weak evidence, especially within a single model family. One line each.
...
3. **Uniquely flagged (`[Claude]`, `[Gemini]`, `[Codex]`, or `[Grok]`)** — issues only one reviewer raised. Judge each on merit: a real issue, or a reviewer idiosyncrasy? Legitimate misses get promoted to the action list; shallow observations get dropped with a reason.
...
5. **"I don't know" items** — if any reviewer flagged something you can't confidently adjudicate without verification (docs, tests, running code, reading source), list it separately. Do *not* act on unverified claims just because they sound plausible. Especially true for claims about tool behaviour, platform quirks, and version-sensitive APIs.

**Failure mode — nothing substantive.** If reviewers return generic praise ("looks good overall", "well-structured", "minor suggestions only"), treat it as weak evidence, not validation. Consider whether the brief was too narrow, whether both models share a training prior that isn't being challenged, or whether a seat defaulted to polite confirmation despite the disagree directive. Options: re-invoke with a sharper prompt, read the work yourself and look for things the reviewers missed, or downgrade confidence and say so.

### Phase 3.5: Anonymized cross-examination (optional)

Run this between round-1 reviews and your final synthesis when the round-1 findings disagree, or when a load-bearing finding came from a single reviewer and you want it stress-tested by a different model family. Skip it for clear-cut runs where the reviewers already converged.

1. **Build a per-reviewer findings pool from your attribution map.** Number every round-1 finding (F1, F2, …). For each reviewer R, assemble the list of findings R did **not** author — pooled from the *other* reviewers, with model names stripped and no label saying which other reviewer raised which (R should not be able to tell Gemini's finding from Codex's, or know either by name). Findings every reviewer already raised in round 1 are consensus — drop them from the pools; they don't need cross-examination. Keep your full attribution map privately for Phase 3; the reviewers never see it.

2. **Write one cross-examination prompt** (reused per reviewer, with that reviewer's pool substituted in) to a scratch file, same `mktemp` pattern as Phase 1: "Here are findings from other independent reviews of the same work. For each, judge it on its own merits — correct, overstated, or wrong? Don't assume a finding is right because it's listed. Flag any you'd actively dispute, and note anything important they all missed." Because R only ever sees findings it didn't write, self-recognition is structurally impossible — no need to ask R to ignore its own.

3. **Resume the round-1 reviewers (Mode B, Phase 2B), in a single message with concurrent calls** — the Claude CLI resume, `gemini --resume`, `codex exec resume <UUID>` — each handed *its own* pool from step 1. No re-brief: each reviewer still has its round-1 context, and you're only adding the others' findings. **Grok has no session to resume**: replay it per Phase 2B step 4's Grok bullet, with its pool as the prompt and the persisted `--source` list, and tag its verdicts `[… xr, fresh-with-replay]`. If any other reviewer can't be resumed, run it fresh with its pool as the brief and tag it the same way.

4. **Assemble the peer-review matrix and fold it into Phase 3 with `xr` tags** (cross-review). Each finding now carries verdicts from the reviewers who didn't raise it — e.g. F3 (raised by Gemini) endorsed by both Claude and Codex → `[F3: Gemini, Claude+Codex xr]`, strongly cross-validated; F5 disputed by a cross-judge → `[F5 disputed: Codex xr]`, flag it. A finding that survives challenge from another family is *better-tested*; one that cross-judges push back on is a likely false positive. Per the guardrail above, surviving cross-review makes a finding better-tested, not "confirmed" — silence under cross-review is weaker evidence than an independent raise.
...
- **Surface all session handles (Claude id, Gemini index, Codex UUID) in visible response text after Phase 2A.** Raw outputs scroll out of context; visible transcript text persists. You can't resume a reviewer in Mode B if you can't find their handle.
...
- **Four model families is the default panel.** Claude + Gemini + Codex + Grok — one reviewer per family. Don't add a second reviewer in the same family (e.g. a second Claude); you pay coordination cost for correlated signal. If a reviewer is down, run the panel short and say so rather than backfilling with a same-family duplicate.
...
- **Attribution is mandatory.** Every finding in the synthesis names the reviewers who raised it — `[Claude]`, `[Gemini]`, `[Codex]`, `[Grok]`, a combination, `[All]`, or any of these with an `r2` (round-2) or `xr` (cross-examination) suffix. This lets the user audit your tiebreak and catches the laundering failure mode where reviewer claims get presented as your own conclusions.
...
- **Mode B is only partially validated.** The Codex resume and Gemini `--resume` legs are proven in real use; the Claude CLI-resume leg's mechanics are verified (context round-trip), but no full review round 2 has run on it yet. Surface gaps back to this file rather than working around them silently — the skill gets sharper with every real use.

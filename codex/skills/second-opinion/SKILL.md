...
4. **Write a self-contained brief** that each reviewer can read. For Mode A they read it cold; for Mode B they read it on top of their existing context. Include:
   ...
   - The output format: structured sections, word cap, explicit "no hedging, disagree where warranted, no sycophancy" directive, and **the evidence attestation each seat owes, per `_shared-rules.md` §23** (it is the single source of truth; this file deliberately carries no copy to drift). Paste §23's attestation table into the brief body verbatim rather than cross-referencing it — the reviewer cannot read this directory. §23 carries the receipt enforcement, the spot-check-before-promoting rule, and the evidence-class tiebreak.
...
3. **Codex CLI**, in parallel with the Claude and Gemini calls. **Give the call a five-minute window** — same reason as Gemini.
   ...
   Invocation: the codex line of the **canonical despatch block in `_shared-rules.md` §10**, substituting `<brief>` with the path recorded in Phase 1. Rationale: `--sandbox read-only` gives Codex read-only tool access (it can Read files but not edit them), matching the independent-review intent — the analogue of Gemini's `--policy` deny-file (step 2); `--skip-git-repo-check` lets it run outside a git repository (the target often isn't one); the trailing `-` reads the brief from stdin. Output is plain text and pipe-friendly; a benign `failed to refresh available models` line may appear on stderr under flaky networks and can be ignored.
...
4. **Invoke the reviewers in a single message with concurrent tool calls**, same as Phase 2A — parallel, not sequential:
   ...
   - **Gemini CLI** with a five-minute window, run from the cwd recorded in step 3. The Phase 2A policy file lives in a different shell call and its variable is gone, so **recreate the read-only policy in the same block** (per `_shared-rules.md` §10) rather than referencing a stale one — an empty `--policy` argument is accepted silently and enforces nothing. Substitute `<round2-path>` with the path from step 2, `<index>` with the session index from step 3, and `<root>` with the target's root:
     ```
   ...
   - **Grok** with a five-minute window — **always replay, never resume** (the wrapper stores nothing, so there is no session handle). Re-send the round-2 prompt with the same `--source` list persisted in Phase 2A, plus the round-1 output so the seat carries its own prior findings:
     ...
     ```
     Derive `<round2-grok-out>` from `<round2-path>` with a `.grok-out` suffix, per `_shared-rules.md` §15. Tag every round-2 Grok finding `[Grok r2, fresh-with-replay]` — it is replay by construction, not a true resume, and the synthesis must not present it as one. If the persisted `--source` list is gone, the seat can't be replayed faithfully; drop it and say so rather than reconstructing a narrower source set.

...
5. **Corrections-log fold.** (if `{VAULT}/07 System/Claude Corrections Log.md` carries distilled rules above a `## Log` tail)
   ...
   - Fold each upward: another instance of a known pattern joins that rule's bracketed date list; a genuinely new failure mode becomes a new rule line under the right domain heading. Then remove the verbatim entry from the live file, once you have confirmed it is recoverable — a vault under version control holds it; otherwise keep the entry. Apply the complete confirmed fold through one `locked-edit.sh --replace` operation against the re-read OLD block; never edit the log directly.
...
6. **Session-log archiving (90-day rolling).**
   ...
   - **Duplicates are a finding, not a cleanup.** Never delete or overwrite either copy during this skill. Surface the on-disk duplication in the report and leave both files alone (same contract `$weekly-hygiene` holds for duplicate detection). Under a lock-only vault policy there is no safe in-skill delete/move primitive, so resolution is deferred even if the user confirms the duplicate; do not improvise with raw filesystem commands.
   - **Move per file with `obsidian move` only where the active installation's vault-write policy permits it** (it heals inbound wikilinks). Drive it per **`_shared-rules.md` §24**, which owns this CLI's behaviour and its preconditions. If the active `AGENTS.md` requires every vault mutation to pass through a locking wrapper and no lock-aware move operation exists, **do not move anything**: report the confirmed move set as deferred. Otherwise create one year folder per destination year in the `MOVE` list (`mkdir -p ".../Session Logs/2025"` etc.), then move each file:
     ```bash
...
7. **Skill-library flywheel audit (DRAFT SPEC — heuristics unvalidated; propose, never auto-apply; optional — skip freely on first runs or when time-boxed, noting "flywheel audit skipped" in the report).**
   The library-level layer of the cross-pollination system uses `${CODEX_HOME:-$HOME/.codex}/skills/_shared-patterns.md` as its index. Some installations also maintain a Claude-side skill-edit Stop hook and `~/.claude/cross-pollination.log`; treat those as optional evidence, not Codex prerequisites. Once a quarter, look across *all* installed Codex skills for infrastructure that's been reinvented rather than shared. Borrowed from Voyager's automatic-curriculum idea: the system proposes its own next consolidation. **Status: spec — the grep heuristics below are untested; treat every finding as a lead to confirm, not a verdict.**
   - **Inventory** `${CODEX_HOME:-$HOME/.codex}/skills/*/SKILL.md` and any project-local `.agents/skills/*/SKILL.md` files. Derive the search targets rather than working a fixed list — a hardcoded list drifts into naming only mechanisms already indexed, which makes the "unindexed" branch unreachable and the audit a no-op:
...
9. **Write the quarterly hygiene report:**
   ...
   ```
   Draft outside the vault, then write `{VAULT}/06 Archive/OpenCairn/Quarterly Hygiene Reports/YYYY-QN.md` through `"{VAULT}/.claude/scripts/locked-edit.sh"`: use `--append` when missing; when replacing an existing report, re-read its full contents and use them as literal OLD with `--replace`. Never write the vault report directly.
...
- **Archives session logs:** inventories logs older than 90 days and, only where the active vault-write policy permits link-aware moves, rolls them into `Session Logs/YYYY/`; lock-only installations stop after the confirmed dry run and report the move set as deferred.

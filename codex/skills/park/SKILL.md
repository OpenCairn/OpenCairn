...
(d) **Pre-log propagation pass:** Step 3 has not written the log yet, so stale living content is reconciled here in editing mode rather than recalled later in record-keeping mode. Build the initial Step 6 enumeration now: every `old → new` pair plus, for each named-entity claim, the entity's canonical identifiers. Run each search yourself in Step 6's exact shape (working directory `{VAULT}`, one `rg -F --type md -i` call per identifier with Step 6's exclusion globs), reconcile every stale living hit per §12 through `locked-edit.sh`, and report `Pre-log propagation: I identifiers, H hits, U updated`. For a substantive reframe (scope, timeline, audience, constraint, underlying premise), also list the affected project's sibling artefacts (files in its folder, files its hub links, live planning lines that reference it) and read each one's framing for a premise the reframe invalidated: framing only, not the Step 4c read; report `Sibling framing: S reviewed, U updated`. This is the initial enumeration Step 4b despatches; the seat's sweep and Step 8's final delta remain the backstop, not the primary catch. **New-entity check** (same pass, the complementary failure class: duplication rather than staleness): list every proper noun the session introduced as a new vault concept (person, project, organisation, product, venue, event, place) and run one `rg -F --type md -i -- '<name>' .` call per name from `{VAULT}` across the whole vault, `06 Archive/` included, since a prior mention under another framing is exactly what this catches. Name each existing home found and merge or cross-link rather than leaving a parallel entry. Report `New entities: E searched` with the hit count per name, zeros included.
...
- **Local reference artefacts:** verify the helper's source hash and metadata, then inspect only declared passages supporting session claims. Never read a whole manual/textbook merely because it was touched. A complete extraction is an index, not evidence every page was read; `empty` or `unsupported` text status proves no text coverage, so use targeted rendering/OCR only when a declared target needs it.
- **Across semantic coverage:** fix broken syntax/links/paths, stale interim state, redundancy, typos and locale spelling (if `.claude/scripts/locale-lint.sh` exists, run it over the ledger's Write/Edit rows — never the mtime backstop's — and fold accepted candidates in, always skipping candidates inside URLs, quoted spans, or capitalised runs; record candidate, applied and skipped counts, including zero), draft-era filename residue after a terminal status change (rename via link-healing move, never raw `mv`), and hook collateral on verbatim external text (§14). A durable new doc must be linked from a durable parent. Don't auto-revert changes you didn't make — surface them.
...
- **File-creation code:** regression-test both mode branches: a missing target receives the caller's umask-derived mode, while replacement of an existing target preserves that target's mode.
- **Superseded queues:** compare the captured pre-state with the retained canonical source before deleting or relabelling the queue. Preserve execution-critical detail; a surviving source label is not evidence that the detail survived.
- **Status narrowing:** bind claims such as "already in place" or "complete" to the exact surface checked and capture the supporting receipt. Count-only or subset evidence cannot discharge a wider status.
...
**One accepted-FAIL class:** a lint hit in content the session never wrote, where the file's classification receipt proves every session delta was an exact declared locator/token substitution, carries pre/post lint fingerprints proving the edit introduced no lint, and records `--allow-inherited-lint`. "Fixing" inherited lint means reformatting the user's own notes, which a standing instruction forbids. Anything the session introduced or any file lacking that mechanical proof is not in this class.

When this is the verifier's **only** failure, rerun through `run-verifier` with one `--accept-inherited-lint "<path>"` before `--` for every affected file. The wrapper accepts only paths pre-approved by those mechanical receipts, an exact path-set match over the complete multi-line lint block, exactly one `FAIL lint`, zero `REVIEW` lines, and `RESULT: FAIL (1 fail, 0 review)`; any additional or differently scoped issue remains a failing receipt.
...
Resolve the task name against `collaboration.list_agents` before spawning. On a collision, append `_addendum_1` and increment the suffix until unused; if the spawn itself reports a collision, recompute and retry. A failed spawn creates no reviewer, so this still yields exactly one fresh seat. Carry the resolved task name into collection and the audit receipt.

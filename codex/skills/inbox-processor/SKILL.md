...
1. **Scan the inbox:**
   ...
   - Read markdown/text notes in full. **Open images with the available image viewer rather than guessing from filename** — capture filenames are usually hashes or app-generated strings with zero routing signal. For other binaries (PDFs, audio, installers, archives), classify from filename + metadata; extract content only where cheap tooling exists (e.g. `pdftotext`). **Never execute a binary.**

...
2. **Load context**
   ...
   **Topic-relevant context** (as needed):
   - Follow the active AGENTS.md context-routing table based on message content, if one is defined; otherwise use the context files it names. If they're asking about travel, load travel files. If discussing a project, load the project file.
...
3. **Draft**
   ...
   **Rules:**
   - No AI disclosure.
   - Locale-appropriate spelling (en_AU or the user's configured locale from AGENTS.md).
...
4. **Voice check (via `$de-ai-ify`)**
   ...
   After drafting, read `~/.codex/skills/de-ai-ify/SKILL.md` completely and apply its workflow to the draft text. `$de-ai-ify` will present the before/after comparison and apply its full checklist. Do not duplicate its logic here.
...
5. **Output**
   ...
   **Default: write to scratchpad** (unless the user overrides — below):
   ...
   - Write via `locked-edit.sh` (`--append` for a new section), never a raw edit — Scratchpad is a shared surface mutated by multiple skills (`_shared-rules.md` §11 locking applies to all Scratchpad mutations, this write included).
